...
class BudgetManager(AgentComponent):
    ...
    async def create_budget(self, 
        ...
        # Generar ID único
        budget_id = f"BUD-{datetime.now().strftime('%Y%m%d')}-{len(self._active_budgets) + 1}"
        ...
        # Emitir evento
        await self.events.emit("budget_created", {
            "budget_id": budget_id,
    ...
    async def update_budget(self, 
        ...
        # Guardar historial
        await self.storage.create("budget_history", {
            "budget_id": budget_id,
            **history_entry
        })
        
        # Emitir evento
        await self.events.emit("budget_updated", {
            "budget_id": budget_id,
            "changes": changes,
            "reason": reason
        })
    ...
    async def handle_package_update(self, event: Dict[str, Any]) -> None:
        ...
        affected_budgets = [
            budget for budget in self._active_budgets.values()
            ...
            try:
                ...
                if analysis.impact_level > 0.7:  # Impacto alto
                    ...
                    await self.update_budget(
                        budget["id"],
                        {"price_adjustment": self._calculate_adjustment(budget, changes)},
                        "package_update"
                    )
                ...
                self.logger.error(f"Error procesando presupuesto {budget['id']}: {str(e)}")
                continue
    ...
    async def handle_package_deletion(self, event: Dict[str, Any]) -> None:
        ...
        for budget in affected_budgets:
            ...
            await self.update_budget(
                ...
                {
                    "status": "invalid",
                    "invalidation_reason": "package_deleted"
                },
                "package_deleted"
            )
            
            # Notificar
            await self.events.emit("budget_invalidated", {
                "budget_id": budget["id"],
                "reason": "package_deleted"
            })

    async def recalculate_budget(self, 
                               budget_id: str,
                               new_package_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _calculate_base_price(self,
                            package: TravelPackage,
                            criteria: SearchCriteria,
                            customer: CustomerProfile) -> Decimal:
    ...
    def _analyze_impact(self,
                       budget: Dict[str, Any],
                       changes: Dict[str, Any]) -> AnalysisResult:
