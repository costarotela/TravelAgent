...
class BudgetBuilder:
    ...
    def __init__(
        self, version_manager: VersionManager, history_manager: HistoryManager
    ):
    ...
    async def reconstruct_budget(
        self, session_id: str, target_version: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def apply_changes(
        self, session_id: str, base_version: str, changes: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Aplicar cambios sobre una versión base."""
        try:
            # Obtener versión base
            budget = await self.reconstruct_budget(
                session_id, target_version=base_version
            )
            ...
            for change in changes:
                if change["change_type"] == "add_package":
                    ...
                    budget["packages"] = [
                        p for p in budget["packages"] if p["id"] != pkg_id
                    ]
    ...
    async def create_checkpoint(
        ...
        try:
            ...
            return await self.version_manager.create_version(
                session_id, budget_data, metadata
            )
