...
class ChangeDetector:
    ...
    def detect_changes(
        ...
        try:
            ...
            if old_data["precio"] != new_data["precio"]:
                ...
                CHANGE_MAGNITUDE.labels(change_type="precio").observe(
                    float(diferencia_porcentual)
                )
            ...
            if old_data["impuestos"] != new_data["impuestos"]:
                ...
                diferencia_porcentual = (
                    abs(diferencia / old_data["impuestos"] * 100)
                    if old_data["impuestos"] > 0
                    else 100
                )
                ...
                CHANGE_MAGNITUDE.labels(change_type="impuestos").observe(
                    float(diferencia_porcentual)
                )

            # Detectar cambios en políticas de cancelación
            if self._detect_policy_changes(
                old_data.get("politicas", []), new_data.get("politicas", [])
            ):
                changes["politicas"] = {
            ...
            if "assist_card" in old_data and "assist_card" in new_data:
                assist_changes = self._detect_assist_card_changes(
                    old_data["assist_card"], new_data["assist_card"]
                )
                self.error_handler.log_operation(
                    ...
                    changes=(
                        json.dumps(assist_changes, default=str)
                        if assist_changes
                        else "None"
                    ),
                )
            ...
            if changes:
                self.error_handler.log_operation(
                    "changes_detected", changes=json.dumps(changes, default=str)
                )
    ...
    def is_significant_change(self, changes: Dict) -> bool:
        ...
        if "precio" in changes:
            if (
                changes["precio"]["diferencia_porcentual"]
        ...
        if "impuestos" in changes:
            if (
                changes["impuestos"]["diferencia_porcentual"]
                > self.significant_change_threshold
            ):
                return True
        ...
        if "assist_card" in changes:
            if "cobertura" in changes["assist_card"]:
                if (
                    changes["assist_card"]["cobertura"]["diferencia_porcentual"]
                    > self.significant_change_threshold
                ):
                    return True
            # Otros cambios en assist_card son significativos
            if any(
                field in changes["assist_card"]
    ...
    def _detect_policy_changes(
        self, old_policies: List[Dict], new_policies: List[Dict]
    ) -> bool:
    ...
    def _detect_assist_card_changes(
        self, old_assist: Dict, new_assist: Dict
    ) -> Optional[Dict]:
        ...
        try:
            ...
            if old_assist["cobertura"] != new_assist["cobertura"]:
                ...
                CHANGE_MAGNITUDE.labels(change_type="assist_card").observe(
                    float(diferencia_porcentual)
                )
