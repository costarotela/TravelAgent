...
class RealtimeValidator(AgentComponent):
    ...
    async def validate_price_change(
        ...
        try:
            ...
            if result['requires_alert']:
                await self.emit_event('significant_price_change', {
            ...
            self.error_handler.log_error('price_validation_error', {
                'error': str(e),
                'package_id': package_id,
                'old_price': str(old_price),
                'new_price': str(new_price)
    ...
    async def validate_availability_change(
        ...
        try:
            ...
            # Registrar métricas
            if dates_added:
                AVAILABILITY_CHANGES.labels(
                    change_type='dates_added'
                ).inc()
                
            if dates_removed:
                AVAILABILITY_CHANGES.labels(
                    change_type='dates_removed'
                ).inc()
            ...
            if result['requires_alert']:
                await self.emit_event('availability_change', {
                    'package_id': package_id,
                    'dates_removed': dates_removed,
                    'dates_added': dates_added
                })
            ...
            self.error_handler.log_error('availability_validation_error', {
                'error': str(e),
                'package_id': package_id
            })
            raise
    ...
    async def process_notification_queue(self):
        ...
        while True:
            try:
                ...
                self.error_handler.log_error('notification_processing_error', {
                    'error': str(e)
                })
