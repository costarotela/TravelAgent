...
class DataValidator:
            ...
            for escala in data["escalas"]:
                ...
                if "ubicacion" not in escala or "tiempo_espera" not in escala:
                    raise ValidationError(
                        "Escala debe tener ubicación y tiempo de espera"
                    )
    ...
    def validate_cancellation_policies(self, policies: List[Dict]) -> None:
        ...
        for policy in policies:
            ...
            if (
                not isinstance(policy["penalidad"], str)
                or not policy["penalidad"].strip()
            ):
    ...
    def validate_assist_card(self, data: Dict[str, Any]) -> None:
        ...
        for field in required_fields:
            if field not in data:
                raise ValidationError(
                    f"Campo de Assist Card requerido faltante: {field}"
                )
        ...
        if (
            not isinstance(data["territorialidad"], str)
            or not data["territorialidad"].strip()
        ):
            raise ValidationError("Territorialidad inválida")

        if (
            not isinstance(data["limitaciones"], str)
            or not data["limitaciones"].strip()
        ):
            raise ValidationError("Limitaciones inválidas")

        if (
            not isinstance(data["gastos_reserva"], Decimal)
            or data["gastos_reserva"] < 0
        ):
