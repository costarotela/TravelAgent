...
class PriceMonitor:
    ...
    async def monitor_package(
        self, package: TravelPackage, track_changes: bool = True
    ) -> List[PriceAlert]:
        """
    ...
    async def start_monitoring(
        self, packages: List[TravelPackage], interval: int = 3600  # 1 hora por defecto
    ):
        """
    ...
    async def _get_price_history(
        self, package: TravelPackage, days: int = 30
    ) -> List[Dict[str, Any]]:
        """Obtener histórico de precios."""
        try:
            ...
            history = await self.memory.get_price_history(
                package_id=package.id, provider=package.provider, since=since
            )
    ...
    def _analyze_price_changes(
        self, package: TravelPackage, price_history: List[Dict[str, Any]]
    ) -> List[PriceAlert]:
