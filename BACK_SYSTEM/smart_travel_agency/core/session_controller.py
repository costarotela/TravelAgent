...
class SessionController:
    ...
    async def create_session(
        ...
        try:
            ...
            session_id = await self.budget_manager.create_session(vendor_id, customer_id)
    ...
    async def add_package(self, session_id: str, package: TravelPackage) -> bool:
        ...
        analysis = await self.package_analyzer.analyze_package(
            package=package,
            criteria=self.active_sessions[session_id]["criteria"].dict() 
            if self.active_sessions[session_id]["criteria"] else {}
        )
