...
class BudgetPanelController:
    ...
    async def handle_websocket_message(self, session_id: str, message: Dict[str, Any]):
        ...
        if msg_type == "modify_package":
            await self.modify_package(
                session_id, message["package_id"], message["modifications"]
            )
    ...
    async def modify_package(
        self, session_id: str, package_id: str, modifications: Dict[str, Any]
    ):
        """Modificar un paquete."""
        # Aplicar modificaciones
        await self.budget_manager.add_modification(
            session_id,
            {
        ...
        await self.notify_session_clients(
            session_id,
            {
                "type": "update",
                "content": {
    ...
    async def apply_recommendation(self, session_id: str, recommendation_id: str):
        ...
        # Aplicar modificaciones
        await self.modify_package(
            session_id, modifications["package_id"], modifications["changes"]
        )
    ...
    async def preview_alternative(self, session_id: str, alternative_id: str):
        ...
        # Notificar a clientes
        await self.notify_session_clients(
            session_id, {"type": "preview", "content": alternative.dict()}
        )
...
async def get_budget_panel(
    session_id: str, controller: BudgetPanelController = Depends()
):
...
async def budget_websocket(
    websocket: WebSocket, session_id: str, controller: BudgetPanelController = Depends()
):
