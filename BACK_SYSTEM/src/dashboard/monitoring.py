...
def render_metrics_overview(df: pd.DataFrame):
    ...
    with col1:
        st.metric("Tiempo Promedio de Búsqueda", f"{avg_search_time:.2f}s")
...
def render_search_times_chart(df: pd.DataFrame):
    ...
    if not search_times.empty:
        ...
        fig.update_layout(xaxis_title="Fecha/Hora", yaxis_title="Duración (segundos)")
...
def render_provider_stats(df: pd.DataFrame):
    ...
    if not provider_stats.empty:
        ...
        stats = provider_stats.groupby(["provider", "metric_name"])[
            "metric_value"
        ].sum()
...
def render_error_log(errors_df: pd.DataFrame):
    ...
    if not errors_df.empty:
        ...
        errors_display["timestamp"] = errors_display["timestamp"].dt.strftime(
            "%Y-%m-%d %H:%M:%S"
        )

        st.dataframe(
            errors_display[
                ["timestamp", "provider", "action", "error_type", "error_message"]
            ],
...
def render_monitoring_dashboard():
    ...
    days = st.sidebar.slider(
        "Período de Análisis (días)", min_value=1, max_value=30, value=7
    )
