...
class BudgetReconstructionManager(AgentComponent):
    ...
    async def initialize(self) -> None:
        ...
        strategies = await self.storage.find("reconstruction_strategies", {"status": "active"})
    ...
    async def shutdown(self) -> None:
        ...
        if self._reconstruction_tasks:
            await asyncio.gather(*self._reconstruction_tasks.values(), return_exceptions=True)
    ...
    async def reconstruct_budget(
        ...
        if not strategy:
            strategy = impact.recommendations[0] if impact.recommendations else ReconstructionStrategy.ADJUST_PROPORTIONALLY

        reconstruction_method = self._get_reconstruction_method(strategy)
        
        updated_budget = await reconstruction_method(budget_id, changes, impact.model_dump())
        
        await self.events.emit("budget_reconstructed", {
            "budget_id": budget_id,
    ...
    async def _handle_high_impact(self, event_data: Dict[str, Any]) -> None:
        ...
        if alternatives:
            await self.events.emit("alternatives_found", {
                "budget_id": budget_id,
    ...
    async def _process_budget_update(
        ...
        try:
            ...
            if impact.impact_level >= 0.7:  # Impacto alto
                ...
                await self.events.emit("budget_impact_high", {
                    "budget_id": budget_id,
                    "changes": changes,
                    "impact": impact.model_dump()
                })
            ...
            self.logger.error(
                f"Error processing budget update {budget_id}: {str(e)}"
            )
        finally:
            if budget_id in self._reconstruction_tasks:
    ...
    async def _reconstruct_best_alternative(
        ...
        updated_budget["base_price"] = Decimal(str(best_alternative["package"]["price"]))
        updated_budget["final_price"] = updated_budget["base_price"] * (1 + budget["margin"])
    ...
    def _calculate_package_similarity(
        self,
        original: TravelPackage,
        alternative: TravelPackage
    ) -> float:
        """
