...
optimization_time = metrics.record_time('optimization_time', 0.0, module='price_optimizer')  # Se actualizará en el método optimize
...
class PriceOptimizer:
        ...
        try:
            ...
            for package in packages:
                try:
                    ...
                    self.logger.error(f"Error optimizando precio para {package.id}: {str(e)}")
                    continue
    ...
    async def forecast_demand(
        ...
        try:
            ...
            confidence = float(np.clip(np.mean(cv_scores), 0, 1))  # Asegurar entre 0 y 1
    ...
    async def get_seasonality_factor(
        self,
        date: datetime,
        destination: str
    ) -> float:
    ...
    async def _competitive_pricing(
        ...
        try:
            ...
            margin = min(max(margin, self.config["min_margin"]), self.config["max_margin"])
    ...
    async def _dynamic_pricing(
        ...
        try:
            ...
            margin = factors.margin + (
                (factors.demand + factors.competition + factors.seasonality - 1.5) / 3 *
            ...
            seasonality_factor = 1.0 + (
                (factors.seasonality - 0.5) *
                self.config["seasonality_weight"]
            )
    ...
    async def _extract_price_factors(
        self,
        package: TravelPackage
    ) -> PriceFactors:
        ...
        try:
            ...
            quality_score = (package.hotel.stars / 5 + package.hotel.review_score / 10) / 2
            ...
            if competitors:
                ...
                competition = sum(
                    ...
                    if comp.total_price < package.total_price
                ) / len(competitors)
            else:
    ...
    async def _calculate_base_cost(
        self,
        package: TravelPackage
    ) -> float:
        """Calcular costo base."""
