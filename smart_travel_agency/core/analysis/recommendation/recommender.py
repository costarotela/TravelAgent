...
class RecommendationEngine:
    ...
    async def generate_recommendations(
        ...
        try:
            ...
            # Registrar operación
            self.metrics.record_operation('recommendation_requests_total', request_type='personalized')
            
            if not available_packages:
                available_packages = await self._fetch_packages(
                    profile
                )
            ...
            # Vectorizar paquetes
            package_vectors = await self._vectorize_packages(
                available_packages
            )
    ...
    async def update_profile(
        ...
        try:
            # Registrar operación
            self.metrics.record_operation('recommendation_requests_total', request_type='profile_update')
            
            # Actualizar preferencias
            if "viewed_package" in interaction:
                await self._update_preferences(
                    profile,
                    interaction["viewed_package"]
                )
    ...
    async def _rank_by_profile(
        ...
        try:
            ...
            for package in packages:
                ...
                interest_match = await self._calculate_interest_match(
                    package,
                    profile
                )
    ...
    async def _calculate_recommendation_score(
        ...
        try:
            ...
            activities_score = vector.activities_score * self.config["activities_weight"]
    ...
    async def _calculate_amenities_score(
        self,
        package: TravelPackage
    ) -> float:
    ...
    async def _calculate_activities_score(
        self,
        package: TravelPackage
    ) -> float:
