...
class BudgetAnalysisEngine:
    ...
    async def analyze_budget(
        self, budget: Budget, criteria: Optional[SearchCriteria] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            insights = await self._generate_insights(
                budget, competitiveness, trends, optimization
            )
            ...
            analysis = {
                ...
                "score": self._calculate_final_score(
                    competitiveness, trends, optimization
                ),
            }

            # Registrar métricas
            duration = (datetime.now() - start_time).total_seconds()
            ANALYSIS_LATENCY.labels(operation_type="full_analysis").observe(duration)
    ...
    async def _analyze_competitiveness(self, budget: Budget) -> Dict[str, Any]:
        ...
        try:
            ...
            for pkg in budget.packages:
                if pkg.id in market_prices:
                    diff = (
                        (pkg.price - market_prices[pkg.id]) / market_prices[pkg.id]
                    ) * 100
            ...
            return {
                "price_difference_avg": (
                    sum(price_differences) / len(price_differences)
                    if price_differences
                    else 0
                ),
                "price_difference_max": (
                    max(price_differences) if price_differences else 0
                ),
    ...
    async def _analyze_trends(self, budget: Budget) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "price_trend_avg": (
                    sum(price_trends) / len(price_trends) if price_trends else 0
                ),
                "demand_trend_avg": (
                    sum(demand_trends) / len(demand_trends) if demand_trends else 0
                ),
                        ...
                        opportunities.append(
                            {
                                ...
                                "recommended_minimum": self.thresholds["margin_minimum"]
                                * 100,
                            }
                        )
                        ...
                        opportunities.append(
                            {
                                ...
                                "recommended_maximum": self.thresholds["price_variance"]
                                * 100,
                            }
                        )
    ...
    def _calculate_final_score(
        ...
        try:
            ...
            trend_score = 100 - (
                abs(trends["price_trend_avg"]) if trends["price_trend_avg"] < 0 else 0
            )
    ...
    def _calculate_opportunity_priority(
        ...
        total_priority = sum(priorities[opp["type"]] for opp in opportunities)
