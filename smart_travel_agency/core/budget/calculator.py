...
class BudgetCalculator:
    ...
    async def calculate_budget(
        ...
        try:
            ...
            if session_state and session_state.get("is_active"):
                packages = await self._get_session_packages(
                    packages,
                    session_state
                )
            ...
            # Aplicar impuestos
            tax_components = await self._apply_taxes(
                margin_components,
                tax_config
            )
            ...
            CALCULATION_LATENCY.labels(
                operation_type="full_calculation"
            ).observe(duration)
            
            CALCULATION_OPERATIONS.labels(
                operation_type="full_calculation"
            ).inc()
        ...
        try:
            ...
            for package in packages:
                ...
                component = PriceComponent(
                    ...
                    total_cost=sum(
                        costs.get("additional_costs", {}).values()
                    ) + costs["base_cost"]
                )
        ...
        try:
            for component in components:
                ...
                if market_data.get("high_demand"):
                    margin = min(
                        market_data["avg_margin"] * 1.2,
                        self.config["max_margin"]
                    )
                else:
                    margin = max(
                        market_data["avg_margin"] * 0.9,
                        self.config["min_margin"]
                    )
    ...
    def _validate_calculation(
        ...
        try:
            ...
            if not (totals["total_cost"] < totals["total_before_tax"] < totals["final_total"]):
                return False
    ...
    async def _get_session_packages(
        ...
        try:
            ...
            isolated_data = await self.memory.get_session_data(
                session_state["id"]
            )
