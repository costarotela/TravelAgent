...
EVENT_COUNTER = Counter(
    "budget_events_total", "Number of budget events processed", ["type"]
)

EVENT_PROCESSING_TIME = Histogram(
    "event_processing_seconds", "Time spent processing events", ["type"]
)
...
class EventManager:
    ...
    def _initialize_processors(self):
        ...
        self.subscribe("budget_changed", self._process_budget_change)

        # Procesador para eventos de reconstrucción
        self.subscribe("budget_reconstructed", self._process_reconstruction)

        # Procesador para eventos de alternativas
        self.subscribe("alternatives_found", self._process_alternatives)
    ...
    async def _process_events(self) -> None:
        ...
        while True:
            try:
                ...
                EVENT_PROCESSING_TIME.labels(type=event["type"]).observe(
                    processing_time
                )

                EVENT_COUNTER.labels(type=event["type"]).inc()
    ...
    async def _process_budget_change(self, event: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            self.logger.info(f"Procesando cambios en presupuesto {budget_id}")
            ...
            if severity >= 0.7:  # 70% o más
                await self._notify_significant_change(budget_id, changes, severity)

            # Actualizar métricas
            self._update_change_metrics(changes)

        except Exception as e:
            self.logger.error(f"Error procesando cambio en presupuesto: {e}")
            raise

    async def _process_reconstruction(self, event: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            self.logger.info(f"Procesando reconstrucción de presupuesto {budget_id}")
            ...
            # Notificar resultado
            await self._notify_reconstruction_result(budget_id, strategy, impact)

        except Exception as e:
            self.logger.error(f"Error procesando reconstrucción: {e}")
            raise

    async def _process_alternatives(self, event: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            self.logger.info(f"Procesando alternativas para presupuesto {budget_id}")
            ...
            # Actualizar métricas
            self._update_alternatives_metrics(len(alternatives), reason)

            # Notificar si hay alternativas viables
            if alternatives:
                await self._notify_alternatives_found(budget_id, alternatives, reason)

        except Exception as e:
            self.logger.error(f"Error procesando alternativas: {e}")
            raise

    def _calculate_change_severity(self, changes: Dict[str, Any]) -> float:
    ...
    def _update_change_metrics(self, changes: Dict[str, Any]) -> None:
        """Actualizar métricas de cambios."""
        for field in changes:
            EVENT_COUNTER.labels(type=f"change_{field}").inc()

    def _update_reconstruction_metrics(
        self, strategy: str, impact: Dict[str, Any]
    ) -> None:
        """Actualizar métricas de reconstrucción."""
        EVENT_COUNTER.labels(type=f"reconstruction_{strategy}").inc()

    def _update_alternatives_metrics(self, count: int, reason: str) -> None:
        """Actualizar métricas de alternativas."""
        EVENT_COUNTER.labels(type=f"alternatives_{reason}").inc()

    async def _notify_significant_change(
        self, budget_id: str, changes: Dict[str, Any], severity: float
    ) -> None:
    ...
    async def _notify_reconstruction_result(
        self, budget_id: str, strategy: str, impact: Dict[str, Any]
    ) -> None:
    ...
    async def _notify_alternatives_found(
        self, budget_id: str, alternatives: List[Dict[str, Any]], reason: str
    ) -> None:
    ...
    async def _record_event_history(self, event_data: Dict[str, Any]) -> None:
