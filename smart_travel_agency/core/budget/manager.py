...
BUDGET_OPERATIONS = Counter(
    'budget_operations_total',
    'Number of budget operations',
    ['operation_type']
)
    ...
    async def optimize_budget(
        ...
        try:
            ...
            for pass_num in range(1, max_passes + 1):
                self.logger.info(f"Starting optimization pass {pass_num} for budget {budget_id}")
                
                # Realizar pasada de optimización
                optimized_data = await self._optimization_pass(budget.base_package, pass_num)
                ...
                result = OptimizationResult(
                    ...
                    changes_applied=self._get_changes(budget.base_package, optimized_data),
                    timestamp=datetime.now()
                )
                
                results.append(result)
                OPTIMIZATION_IMPROVEMENTS.labels(pass_number=pass_num).observe(float(improvement))
                ...
                if improvement > self.optimization_threshold:
                    ...
                    self.logger.info(f"Found improvement of {improvement:.2f}% in pass {pass_num}")
                else:
