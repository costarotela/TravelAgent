...
class NotificationManager:
    ...
    def _initialize_handlers(self):
        ...
        self.register_handler(
            "alternatives",
            self._handle_alternatives_notification
        )
    ...
    async def _handle_price_notification(
        ...
        try:
            ...
            if priority == NotificationPriority.HIGH.value:
                self.logger.warning(
                    f"ALERTA DE PRECIO - {title}: {message}"
                )
                ...
                self.logger.info(
                    f"Cambio de Precio - {title}: {message}"
                )
                
            else:
                self.logger.debug(
                    f"Actualización de Precio - {title}: {message}"
                )
            ...
            self.logger.error(
                f"Error procesando notificación de precio: {e}"
            )
            raise
    ...
    async def _handle_availability_notification(
        ...
        try:
            ...
            if priority == NotificationPriority.HIGH.value:
                self.logger.warning(
                    f"ALERTA DE DISPONIBILIDAD - {title}: {message}"
                )
                ...
                self.logger.info(
                    f"Cambio de Disponibilidad - {title}: {message}"
                )
    ...
    async def _handle_reconstruction_notification(
        ...
        try:
            ...
            if priority == NotificationPriority.HIGH.value:
                self.logger.warning(
                    f"RECONSTRUCCIÓN CRÍTICA - {title}: {message}"
                )
                ...
                self.logger.info(
                    f"Reconstrucción en Proceso - {title}: {message}"
                )
                
            else:
                self.logger.info(
                    f"Reconstrucción Completada - {title}: {message}"
                )
    ...
    async def _handle_alternatives_notification(
        ...
        try:
            ...
            # Registrar en log
            self.logger.info(
                f"Alternativas Encontradas - {title}: {message}"
            )
            ...
            self.logger.error(
                f"Error procesando notificación de alternativas: {e}"
            )
            raise
    ...
    async def _record_notification_history(
        ...
        self.logger.debug(
            f"Notificación registrada: {notification_data}"
        )
