...
class BudgetOptimizer:
    ...
    async def optimize_budget(
        ...
        try:
            ...
            if session_state and session_state.is_active:
                stability_score = self._calculate_stability_impact(budget, goal)
            ...
            for iteration in range(self.config["max_iterations"]):
                ...
                optimized_budget = await optimize_func(current_budget, constraints)
            ...
            if not self._validate_optimization(best_budget, constraints):
                return OptimizationResult(success=False, error="invalid_optimization")

            # Registrar métricas
            duration = (datetime.now() - start_time).total_seconds()
            OPTIMIZATION_LATENCY.labels(operation_type=goal.value).observe(duration)

            OPTIMIZATION_OPERATIONS.labels(operation_type=goal.value).inc()
            ...
            return OptimizationResult(
                ...
                metadata={
                    ...
                    "improvement": (best_score - self._calculate_score(budget, goal))
                    / self._calculate_score(budget, goal),
                },
            )
    ...
    async def _optimize_for_margin(
        self, budget: Budget, constraints: Optional[List[OptimizationConstraint]]
    ) -> Budget:
                ...
                if package.id in quality_data and package.id in market_data:
                    ...
                    if quality_score > 0.8:  # Alta calidad
                        new_price = market_price * (
                            1 + self.config["price_flexibility"]
                        )
    ...
    def _calculate_score(self, budget: Budget, goal: OptimizationGoal) -> float:
        ...
        try:
            if goal == OptimizationGoal.MAXIMIZE_MARGIN:
                ...
                margin_score = self._calculate_score(
                    budget, OptimizationGoal.MAXIMIZE_MARGIN
                )
                competitive_score = self._calculate_score(
                    budget, OptimizationGoal.MAXIMIZE_COMPETITIVENESS
                )
    ...
    def _calculate_stability_impact(
        self, budget: Budget, goal: OptimizationGoal
    ) -> float:
    ...
    def _validate_constraint(
        self, budget: Budget, constraint: OptimizationConstraint
    ) -> bool:
