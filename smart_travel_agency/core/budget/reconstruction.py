...
ACTIVE_RECONSTRUCTIONS = Gauge(
    'active_reconstructions_total',
    'Number of active reconstruction processes'
)
...
class BudgetReconstructionManager:
    ...
    async def _reconstruct_preserve_margin(
        ...
        try:
            ...
            if "price" in changes:
                price_change = changes["price"]
                new_cost = original_cost * (1 + Decimal(str(price_change["percentage"])) / 100)
            else:
    ...
    async def _reconstruct_adjust_proportionally(
        ...
        try:
            ...
            if "price" in changes:
                ...
                margin_impact = change_percentage * Decimal('0.4')  # 40% del impacto al margen
