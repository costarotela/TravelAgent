...
from ...schemas import APIConfig, APICredentials, APIResponse, RateLimitConfig
...
class BaseAPIClient(ABC):
    ...
    def __init__(self, config: APIConfig, credentials: APICredentials):
        """
        Inicializar cliente.
        ...
        self.rate_limiter = RateLimitConfig(
            calls=config.rate_limit.calls, period=config.rate_limit.period
        )

    async def __aenter__(self):
        """Inicializar cliente."""
        self.session = aiohttp.ClientSession(
            base_url=self.config.base_url, headers=self.config.default_headers
        )
        return self
    ...
    async def request(self, method: str, endpoint: str, **kwargs) -> APIResponse:
        ...
        try:
            ...
            async with self.session.request(
                ...
                API_LATENCY.labels(
                    api_name=self.config.name, operation=endpoint
                ).observe(duration)
                ...
                return APIResponse(
                    ...
                    metadata={"duration": duration, "endpoint": endpoint},
                )
            ...
            API_OPERATIONS.labels(
                api_name=self.config.name, operation=endpoint, status="error"
            ).inc()
...
class APIManager:
    ...
    def register_client(self, name: str, client_class: Type[BaseAPIClient]) -> None:
        """
    ...
    async def get_client(
        self, name: str, config: APIConfig, credentials: APICredentials
    ) -> BaseAPIClient:
        """
        Obtener cliente API.
    ...
    async def authenticate(self) -> None:
        ...
        try:
            ...
            if not response.success:
                raise Exception(f"Error de autenticación: {response.error}")
