...
class ChangeAnalyzer:
    ...
    def analyze_impact(self, change: Dict[str, Any]) -> ChangeImpact:
        ...
        if 'disponibilidad' in change:
            ...
            if removed > added:
                impact = max(impact, 10.0)  # Alto impacto si hay pérdida de disponibilidad
    ...
    def _generate_recommendation(self, impact: float, potential: float, affected: List[str]) -> str:
...
class ChangeDetector:
    ...
    def detect_changes(self, 
                      ...
                      pass_number: Optional[int] = None) -> Tuple[Dict[str, Any], ChangeImpact]:
        """
        ...
        if old_data['precio'] != new_data['precio']:
            diferencia = Decimal(str(new_data['precio'])) - Decimal(str(old_data['precio']))
            diferencia_porcentual = abs(diferencia / Decimal(str(old_data['precio'])) * 100)
            ...
            CHANGES_DETECTED.labels(
                change_type='precio',
                optimization_pass=self.current_pass
            ).inc()
    ...
    def should_continue_optimization(self, impact: ChangeImpact) -> bool:
        ...
        """
        return (
            impact.optimization_potential > self.optimization_threshold and
            'OPTIMIZAR' in impact.recommendation
        )
