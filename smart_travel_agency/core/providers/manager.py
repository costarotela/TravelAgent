...
ACTIVE_MONITORS = Gauge(
    ...
    'Number of active package monitors',
    ['provider_id']
)
...
class ProviderIntegrationManager:
                ...
                if changes:
                    if 'price' in changes:
                        PRICE_CHANGES.labels(
                            provider_id=provider_id
                        ).observe(changes['price']['percentage'])
    ...
    async def _search_provider(
        ...
        try:
            ...
            if results and optimization_enabled:
                ...
                results = await self._optimize_results(
                    provider_id,
                    results
                )
    ...
    def _detect_changes(
        ...
        # Cambios en precio
        if abs((current.price - previous.price) / previous.price) > self.min_price_change:
            changes["price"] = {
