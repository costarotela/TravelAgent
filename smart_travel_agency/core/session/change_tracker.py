...
from ..schemas import SessionState, Budget, TravelPackage, ChangeRecord, ChangePattern
from ..metrics import get_metrics_collector
from .state_manager import get_session_manager

# Métricas
CHANGE_OPERATIONS = Counter(
    "change_operations_total", "Number of change operations", ["change_type"]
)
...
class ChangeTracker:
    ...
    async def record_change(
        ...
        try:
            ...
            # Registrar métrica
            CHANGE_OPERATIONS.labels(change_type=change_type).inc()
    ...
    async def get_patterns(self, session_id: str) -> List[ChangePattern]:
        """
    ...
    async def analyze_session(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if changes:
                duration = (
                    changes[-1].timestamp - changes[0].timestamp
                ).total_seconds()
            else:
                duration = 0
            ...
            return {
                ...
                "changes_per_minute": (
                    len(changes) / (duration / 60) if duration > 0 else 0
                ),
            }
    ...
    async def _analyze_patterns(self, session_id: str) -> None:
        ...
        try:
            ...
            for i in range(len(window) - 1):
                for j in range(i + 2, len(window) + 1):
                    sequence = tuple(c.change_type for c in window[i:j])
