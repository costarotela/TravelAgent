...
STABILITY_SCORE = Gauge(
    "session_stability_score", "Current stability score of session", ["session_id"]
)

STABILITY_VIOLATIONS = Counter(
    "stability_violations_total", "Number of stability violations", ["violation_type"]
)
...
class StabilityGuard:
    ...
    async def validate_change(
        self, session_id: str, proposed_budget: Budget
    ) -> ChangeImpact:
        """
        ...
        try:
            ...
            if not session or not session.is_active:
                return ChangeImpact(
                    is_safe=False, stability_score=0.0, violations=["invalid_session"]
                )

            # Calcular métricas de estabilidad
            metrics = await self._calculate_metrics(session.budget, proposed_budget)

            # Actualizar métricas
            self._stability_metrics[session_id] = metrics

            # Registrar métricas
            STABILITY_SCORE.labels(session_id=session_id).set(metrics.stability_score)

            # Validar violaciones
            violations = await self._check_violations(metrics)

            for violation in violations:
                STABILITY_VIOLATIONS.labels(violation_type=violation).inc()
            ...
            return ChangeImpact(is_safe=False, stability_score=0.0, violations=[str(e)])
    ...
    async def monitor_session(self, session_id: str) -> None:
        ...
        try:
            ...
            if metrics.stability_score < self.config["critical_threshold"]:
                ...
                await self._notify_critical_stability(session_id, metrics)

        except Exception as e:
            self.logger.error(f"Error monitoreando sesión: {e}")

    async def _calculate_metrics(
        self, current_budget: Budget, proposed_budget: Budget
    ) -> StabilityMetrics:
        """Calcular métricas de estabilidad."""
        try:
            metrics = StabilityMetrics(
                price_changes=[], margin_changes=[], stability_score=1.0
            )
            ...
            for old_pkg, new_pkg in zip(
                current_budget.packages, proposed_budget.packages
            ):
                if old_pkg.price > 0:
                    price_change = abs((new_pkg.price - old_pkg.price) / old_pkg.price)
            ...
            if metrics.price_changes:
                price_factor = 1 - (
                    max(metrics.price_changes) / self.config["max_price_change"]
                )
            else:
                price_factor = 1.0
            ...
            if metrics.margin_changes:
                margin_factor = 1 - (
                    max(metrics.margin_changes) / self.config["max_margin_change"]
                )
            else:
                margin_factor = 1.0
            ...
            return StabilityMetrics(
                price_changes=[], margin_changes=[], stability_score=0.0
            )

    async def _check_violations(self, metrics: StabilityMetrics) -> List[str]:
    ...
    async def _notify_critical_stability(
        self, session_id: str, metrics: StabilityMetrics
    ) -> None:
