...
from ...schemas import Budget, TravelPackage, EditOperation, ValidationResult
from ...metrics import get_metrics_collector
from ...core.budget import get_budget_calculator, get_budget_optimizer
...
VALIDATION_TIME = Histogram(
    "budget_validation_seconds", "Time spent validating budget changes"
)
...
class BudgetEditor:
    ...
    def _register_routes(self) -> None:
        ...
        async def validate_changes(
            budget_id: str, changes: List[EditOperation]
        ) -> ValidationResult:
            """Validar cambios propuestos."""
            return await self.validate_changes(budget_id, changes)

        @self.router.post("/apply")
        async def apply_changes(budget_id: str, changes: List[EditOperation]) -> Budget:
        ...
        async def undo_changes(budget_id: str) -> Budget:
            """Deshacer últimos cambios."""
            return await self.undo_changes(budget_id)

    async def validate_changes(
        self, budget_id: str, changes: List[EditOperation]
    ) -> ValidationResult:
        """
        Validar cambios propuestos.
        ...
        try:
            ...
            # Verificar cálculos
            calc_result = await calculator.calculate_budget(modified_budget.packages)

            if not calc_result.success:
                return ValidationResult(
                    valid=False, errors=["Error en cálculos: " + calc_result.error]
                )

            # Verificar optimización
            opt_result = await optimizer.validate_budget(calc_result.budget)

            if not opt_result.success:
                return ValidationResult(
                    valid=False, errors=["Error en optimización: " + opt_result.error]
                )
            ...
            return ValidationResult(valid=True, warnings=opt_result.warnings)

        except Exception as e:
            self.logger.error(f"Error validando cambios: {e}")

            EDIT_OPERATIONS.labels(operation_type="validate", status="error").inc()
    ...
    async def apply_changes(
        ...
        try:
            # Validar primero
            validation = await self.validate_changes(budget_id, changes)

            if not validation.valid:
                raise ValueError("Cambios inválidos: " + ", ".join(validation.errors))
            ...
            success = await session_manager.update_session(
                budget_id, session.budget, "Applied changes from editor"
            )
            ...
            # Registrar operación
            EDIT_OPERATIONS.labels(operation_type="apply", status="success").inc()
            ...
            EDIT_OPERATIONS.labels(operation_type="apply", status="error").inc()
    ...
    async def generate_preview(
        ...
        try:
            ...
            # Calcular resultados
            calculator = await get_budget_calculator()
            result = await calculator.calculate_budget(preview_budget.packages)

            if not result.success:
                raise ValueError("Error calculando preview")

            # Registrar operación
            EDIT_OPERATIONS.labels(operation_type="preview", status="success").inc()
            ...
            EDIT_OPERATIONS.labels(operation_type="preview", status="error").inc()

            raise HTTPException(status_code=400, detail=str(e))

    async def undo_changes(self, budget_id: str) -> Budget:
        """
        Deshacer últimos cambios.
        ...
        try:
            ...
            # Registrar operación
            EDIT_OPERATIONS.labels(operation_type="undo", status="success").inc()
            ...
            EDIT_OPERATIONS.labels(operation_type="undo", status="error").inc()

            raise HTTPException(status_code=400, detail=str(e))

    async def _apply_operations(
        self, budget: Budget, operations: List[EditOperation]
    ) -> None:
        ...
        try:
            for operation in operations:
                if operation.type == "update_package":
                    # Actualizar paquete
                    package = next(
                        (p for p in budget.packages if p.id == operation.target_id),
                        None,
                    )

                    if not package:
                        raise ValueError(f"Paquete {operation.target_id} no encontrado")
