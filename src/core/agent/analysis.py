...
class AnalysisSystem:
    ...
    async def analyze_route(
        ...
        with db.get_session() as session:
            ...
            packages = session.query(TravelPackage).filter(
                TravelPackage.origin == origin,
                TravelPackage.destination == destination,
                TravelPackage.created_at >= datetime.utcnow() - timedelta(days=days)
            ).order_by(TravelPackage.created_at.asc()).all()
            ...
            analyses = session.query(MarketAnalysis).filter(
                ...
                MarketAnalysis.analysis_date >= datetime.utcnow() - timedelta(days=days)
            ).order_by(MarketAnalysis.analysis_date.asc()).all()
    ...
    def _analyze_seasonality(self, dates: List[datetime], prices: List[float]) -> str:
        ...
        avg_price = sum(prices) / len(prices)
        max_variation = max(
            abs(price - avg_price) / avg_price * 100
            for price in daily_avgs.values()
        )
    ...
    def _generate_recommendation(
        ...
        if price_trend in ["strongly_bearish", "bearish"] and demand_trend in ["high", "very_high"]:
            return "strong_buy"
        elif price_trend in ["strongly_bullish", "bullish"] and demand_trend in ["low", "very_low"]:
            return "strong_sell"
        elif volatility > 20:
            return "high_risk"
