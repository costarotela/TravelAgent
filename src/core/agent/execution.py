...
from src.core.agent.planning import (
    planning_system,
    ActionType,
    Action,
    Plan,
...
class ExecutionSystem:
    ...
    async def execute_action(self, action: Action, plan: Plan) -> TaskExecution:
    ...
    async def _execute_purchase(self, execution: TaskExecution, plan: Plan):
        ...
        with db.get_session() as session:
            ...
            analysis = MarketAnalysis(
                ...
                trend=(
                    "up"
                    if plan.metadata["price_trend"] in ["bullish", "strongly_bullish"]
                    else "down"
                ),
    ...
    async def _execute_sell(self, execution: TaskExecution, plan: Plan):
        ...
        with db.get_session() as session:
            ...
            analysis = MarketAnalysis(
                ...
                trend=(
                    "down"
                    if plan.metadata["price_trend"] in ["bearish", "strongly_bearish"]
                    else "up"
                ),
                analysis_date=datetime.utcnow(),
                data={
