...
class LearningSystem:
    ...
    def __init__(self):
        self._performance_history: Dict[str, List[ActionPerformance]] = defaultdict(list)
    ...
    async def _analyze_action_performance(
        ...
        if similar_actions:
            ...
            success_rate = (sum(prev_success_rates) + float(success)) / (len(prev_success_rates) + 1)
        ...
        risk_accuracy = 1.0 - min(1.0, abs(expected_impact - actual_impact) / expected_impact if expected_impact > 0 else 0.0)
    ...
    async def _update_route_patterns(
        ...
        for execution in executions:
            ...
            if risk_level:
                ...
                risk_data["success_rate"] = (
                    (risk_data["success_rate"] * (risk_data["count"] - 1) + 
                     float(execution.result == ExecutionResult.SUCCESS)) / risk_data["count"]
                )
                if execution.impact is not None:
                    risk_data["avg_impact"] = (
                        (risk_data["avg_impact"] * (risk_data["count"] - 1) + 
                         execution.impact) / risk_data["count"]
                    )
            ...
            patterns["execution_patterns"][execution.action.type.value].append(execution_pattern)
    ...
    def _adjust_confidence_factors(
        ...
        confidence_correlation = np.mean([p.confidence_correlation for p in performances])
    ...
    def _calculate_learning_metrics(
        ...
        confidence_calibration = np.mean([p.confidence_correlation for p in performances])
        ...
        if len(performances) > 1:
            ...
            improvement_rate = (final_success - initial_success) / initial_success if initial_success > 0 else 0.0
        else:
            improvement_rate = 0.0
    ...
    def _get_action_statistics(self, route_key: str) -> Dict[str, Dict]:
        ...
        for action_type, executions in execution_patterns.items():
            ...
            stats[action_type] = {
                ...
                "trend": "improving" if len(impacts) > 1 and impacts[-1] > impacts[0] else "stable"
            }
