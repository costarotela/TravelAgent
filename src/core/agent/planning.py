...
class PlanningSystem:
    ...
    async def create_plan(
        self, origin: str, destination: str, timeframe_days: int = 7
    ) -> Optional[Plan]:
        ...
        insight = await analysis_system.analyze_route(origin, destination, days=30)
        ...
        actions = self._generate_actions(
            insight, market_status, risk_level, timeframe_days
        )
    ...
    def _generate_actions(
        self, insight, market_status: str, risk_level: str, timeframe_days: int
    ) -> List[Action]:
        """Generar acciones recomendadas."""
        actions = []
        now = datetime.utcnow()
