...
from src.core.analysis.confidence_calibrator import (
...
class AdvancedEstimator:
    ...
    def _generate_recommendation(
        self, impact: float, confidence: float, market: MarketCondition
    ) -> str:
    ...
    def _evaluate_risk(
        ...
        risk_score = (
            (1 - confidence) * 0.4 + market.volatility * 0.3 + (1 - impact) * 0.3
        )
    ...
    def update_models(
        ...
        self.confidence_calibrator.update_calibration(
            predicted_confidence, actual_outcome, action_type
        )
