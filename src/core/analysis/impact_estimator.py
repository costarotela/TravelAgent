...
class ImpactEstimator:
    ...
    async def estimate_impact(
        ...
        market_condition = self._analyze_market_conditions(
            origin, destination, market_data
        )

        # Obtener impacto histórico
        historical_impact = await self._get_historical_impact(
            action_type, origin, destination, historical_data
        )

        # Calcular impacto estimado
        estimated_impact = self._calculate_weighted_impact(
            market_condition, historical_impact
        )

        # Calcular confianza
        confidence = self._calculate_confidence(market_condition, historical_impact)
    ...
    def _analyze_market_conditions(
        self, origin: str, destination: str, market_data: Dict
    ) -> MarketCondition:
        ...
        seasonality = self._calculate_seasonality(origin, destination, market_data)
    ...
    async def _get_historical_impact(
        ...
        similar_actions = [
            ...
            and action["origin"] == origin
            and action["destination"] == destination
        ]
        ...
        confidence_level = self._calculate_confidence_level(
            sample_size, success_rate, recency_factor
        )
    ...
    def _calculate_weighted_impact(
        self, market_condition: MarketCondition, historical_impact: HistoricalImpact
    ) -> float:
        ...
        historical_component = (
            historical_impact.avg_impact * historical_impact.recency_factor
        )
    ...
    def _calculate_confidence_level(
        self, sample_size: int, success_rate: float, recency_factor: float
    ) -> float:
    ...
    def _adjust_weights(
        ...
        historical_quality = (
            historical_impact.sample_size / 10.0 * historical_impact.recency_factor
        )
    ...
    def _calculate_market_impact(self, condition: MarketCondition) -> float:
        ...
        trend_factor = {"up": 1.2, "down": 0.8, "stable": 1.0}.get(condition.trend, 1.0)
