...
class RiskAnalyzer:
    ...
    async def analyze_risk(
        ...
        risk_factors = self._extract_risk_factors(
            market_data, financial_data, historical_data
        )

        # Calcular métricas de riesgo
        risk_metrics = self._calculate_risk_metrics(
            action_type, risk_factors, historical_data
        )
        ...
        recommendations = self._generate_risk_recommendations(
            risk_metrics, risk_factors, risk_level
        )
    ...
    def _extract_risk_factors(
        ...
        active_competitors = sum(1 for comp in competitors if comp.get("active", False))
        competition_level = active_competitors / max(len(competitors), 1)

        # Analizar estabilidad de demanda
        demand_history = [d.get("demand", 0.5) for d in (historical_data or [])]
    ...
    def _calculate_risk_metrics(
        self, action_type: str, factors: RiskFactors, historical_data: Optional[List]
    ) -> RiskMetrics:
        """Calcula métricas detalladas de riesgo."""
        # Calcular riesgo de volatilidad
        volatility_risk = (
            factors.market_volatility * 0.7 + factors.price_sensitivity * 0.3
        )
        ...
        total_risk = market_risk * 0.4 + operational_risk * 0.3 + financial_risk * 0.3

        # Calcular tolerancia al riesgo
        risk_tolerance = self._calculate_risk_tolerance(action_type, historical_data)

        # Calcular capacidad de riesgo
        risk_capacity = self._calculate_risk_capacity(total_risk, risk_tolerance)

        # Calcular VaR y CVaR
        returns = self._calculate_historical_returns(historical_data)
        var_95 = np.percentile(returns, 5) if len(returns) > 0 else -0.1
        cvar_95 = (
            np.mean([r for r in returns if r <= var_95]) if len(returns) > 0 else -0.15
        )
    ...
    def _calculate_risk_tolerance(
        ...
        relevant_data = [
            action for action in historical_data if action.get("type") == action_type
        ]
        ...
        success_rate = np.mean(
            [float(action.get("success", False)) for action in relevant_data]
        )
    ...
    def _calculate_risk_capacity(
        self, total_risk: float, risk_tolerance: float
    ) -> float:
    ...
    def _calculate_historical_returns(
        self, historical_data: Optional[List]
    ) -> List[float]:
    ...
    def _generate_risk_recommendations(
        self, metrics: RiskMetrics, factors: RiskFactors, risk_level: str
    ) -> List[str]:
        ...
        if risk_level in ["alto", "muy_alto"]:
            recommendations.append(
                "Nivel de riesgo elevado - " "requiere aprobación y monitoreo especial"
            )
        elif risk_level == "medio":
            recommendations.append(
                "Riesgo moderado - " "implementar controles adicionales"
            )

        return recommendations

    def update_risk_history(
        self, action_type: str, risk_metrics: RiskMetrics, actual_outcome: float
    ) -> None:
    ...
    def get_risk_stats(self) -> Dict:
        ...
        recent_risks = [h["metrics"].total_risk for h in self._risk_history[-50:]]

        return {
            "mean_risk": np.mean(recent_risks),
            "risk_volatility": np.std(recent_risks),
            "risk_trend": (
                "increasing" if np.mean(np.diff(recent_risks)) > 0 else "decreasing"
            ),
