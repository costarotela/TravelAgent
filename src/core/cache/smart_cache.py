...
CACHE_SIZE = Counter('cache_size_bytes', 'Size of cache in bytes')

class SmartCache:
    """
    Caché inteligente que ajusta el tiempo de expiración basado en patrones de cambio.
    """
        ...
        if changed:
            ...
            if key in self._change_patterns:
                ...
                self._change_patterns[key] = (
                    self._change_patterns[key] * 0.7 +
                    time_since_update * 0.3
                )
            else:
