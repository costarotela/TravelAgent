...
class MarketObserver:
    ...
    async def analyze_route(self, origin: str, destination: str) -> Optional[Dict]:
    ...
    def _calculate_demand(self, packages: List[TravelPackage]) -> float:
        ...
        )  # Assuming max availability is 100
    ...
    def _determine_trend(self, packages: List[TravelPackage]) -> str:
        ...
        second_half_avg = sum(p.price for p in sorted_packages[mid:]) / (
            len(sorted_packages) - mid
        )
