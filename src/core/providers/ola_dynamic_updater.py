...
UPDATE_DURATION = Histogram('ola_update_duration_seconds', 'Time spent updating OLA packages')
CHANGES_DETECTED = Counter('ola_changes_detected', 'Number of changes detected in OLA packages')
...
class OLAUpdater:
    ...
    def __init__(
        self,
        config: Dict,
        monitor: Optional[OLAMonitor] = None
    ):
        """
    ...
    def _update_metrics(
        ...
        if price_stats['total_cambios'] > 0:
            ...
            price_metrics = {
                'precio_promedio': sum(c['precio_actual'] for c in changes) / len(changes),
        ...
        self.monitor.update_availability(
            availability['porcentaje_disponible'],
            destino
        )
