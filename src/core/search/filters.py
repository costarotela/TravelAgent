        ...
        try:
            ...
            if self.price_range:
                filtered = [
                    ...
                    if self.price_range.min_price <= pkg.price <= self.price_range.max_price
                ]
                filters_applied += 1
            ...
            if self.flight_types:
                filtered = [
                    pkg for pkg in filtered
                    if pkg.details.get("type", "").lower() in [t.lower() for t in self.flight_types]
                ]
                filters_applied += 1
            ...
            if self.cabin_classes:
                filtered = [
                    pkg for pkg in filtered
                    if pkg.details.get("cabin_class", "").lower() in [c.lower() for c in self.cabin_classes]
                ]
                filters_applied += 1
            ...
            if self.departure_time:
                filtered = [
                    pkg for pkg in filtered
                    if (departure_time := self._parse_time(pkg.details.get("departure_time", ""))) and
                    self.departure_time.start_time <= departure_time <= self.departure_time.end_time
                ]
                filters_applied += 1
            ...
            if self.airlines:
                filtered = [
                    pkg for pkg in filtered
                    if pkg.details.get("airline", "").lower() in [a.lower() for a in self.airlines]
                ]
                filters_applied += 1
            ...
            if self.min_baggage is not None:
                filtered = [
                    pkg for pkg in filtered
                    if (baggage := self._parse_baggage(pkg.details.get("baggage", "0kg"))) and
                    baggage >= self.min_baggage
                ]
                filters_applied += 1
