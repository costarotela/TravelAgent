...
st.set_page_config(
    page_title="Travel Provider Monitor",
    page_icon="✈️",
    layout="wide"
)
        ...
        with main_container:
            ...
            if status_data:
                ...
                with col2:
                    st.metric(
                        "Last Check",
                        datetime.fromisoformat(status_data["last_check"]).strftime("%H:%M:%S"),
                        delta=None
                    )
                ...
                with col3:
                    total_ops = sum(
                        m["total_calls"]
                        for m in status_data["metrics"].values()
                    )
