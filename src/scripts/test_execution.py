...
async def test_execution():
    ...
    for origin, destination in routes:
        ...
        for i, execution in enumerate(executions, 1):
            ...
            print(f"   ✓ Resultado: {execution.result.value if execution.result else 'N/A'}")
            print(f"   ✓ Impacto: {execution.impact:.2f}% si está disponible" if execution.impact else "   ✗ Impacto: No disponible")
            print(f"   ✓ Duración: {(execution.end_time - execution.start_time).total_seconds():.2f}s")
