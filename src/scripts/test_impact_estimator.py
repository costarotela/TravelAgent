...
async def test_impact_estimator():
    ...
    for origin, destination in routes:
        ...
        for action_type in ["purchase", "sell", "investigate"]:
            ...
            print(
                f"   ✓ Estacionalidad: {estimation['market_condition'].seasonality:.2%}"
            )
            print(
                f"   ✓ Nivel de competencia: {estimation['market_condition'].competition_level:.2%}"
            )

            if estimation["historical_impact"].sample_size > 0:
                print("\n   Datos históricos:")
                print(
                    f"   ✓ Impacto promedio: {estimation['historical_impact'].avg_impact:.2%}"
                )
                print(
                    f"   ✓ Tasa de éxito: {estimation['historical_impact'].success_rate:.2%}"
                )
                print(
                    f"   ✓ Nivel de confianza: {estimation['historical_impact'].confidence_level:.2%}"
                )
                print(
                    f"   ✓ Tamaño de muestra: {estimation['historical_impact'].sample_size}"
                )
                print(
                    f"   ✓ Factor de recencia: {estimation['historical_impact'].recency_factor:.2%}"
                )
