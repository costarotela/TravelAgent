...
async def test_update_profile(
    ...
    # Caso 1: Ver paquete
    interaction = {
        "viewed_package": sample_package
    }
    
    updated = await engine.update_profile(
        sample_profile,
        interaction
    )
...
@pytest.mark.asyncio
async def test_vectorize_packages(
    sample_packages
):
    ...
    # Verificar cache
    assert all(
        pkg.id in engine.vector_cache
        for pkg in sample_packages
    )

@pytest.mark.asyncio
async def test_find_similar_packages(
    sample_package,
    sample_packages
):
    ...
    # Vectorizar paquetes
    vectors = await engine._vectorize_packages(
        [sample_package] + sample_packages
    )
    ...
    assert all(
        s1 >= s2
        for s1, s2 in zip(similarities, similarities[1:])
    )

@pytest.mark.asyncio
async def test_rank_by_profile(
    sample_profile,
    sample_packages
):
...
@pytest.mark.asyncio
async def test_recommendation_score(
    sample_package
):
