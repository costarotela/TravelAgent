...
class TestOLAMonitor:
    ...
    def test_update_price_metrics(self, monitor):
        ...
        monitor.precio_minimo.labels.assert_called_with(
            destino=destino,
            moneda=moneda
        )
        monitor.precio_maximo.labels.assert_called_with(
            destino=destino,
            moneda=moneda
        )
    ...
    def test_update_performance_metrics(self, monitor):
        ...
        monitor.tiempo_actualizacion.labels(destino=destino).set.assert_called_with(tiempo)
    ...
    def test_update_scraping_duration(self, monitor):
        ...
        monitor.scraping_duration.labels(destino=destino).set.assert_called_with(duracion)
    ...
    def test_update_availability(self, monitor):
        ...
        monitor.disponibilidad.labels(destino=destino).set.assert_called_with(porcentaje)
