...
def test_validate_flight_success(validator):
    ...
    flight_data = {
        ...
        "duracion": "2 horas",
        "escalas": [{"ubicacion": "Panama", "tiempo_espera": "1 hora"}],
    }
...
def test_validate_flight_missing_field(validator):
    ...
    with pytest.raises(
        ValidationError, match="Campo de vuelo requerido faltante: llegada"
    ):
...
def test_validate_cancellation_policies_success(validator):
    ...
    policies = [
        {"periodo": "Hasta 30 días antes", "penalidad": "10% del total"},
        {"periodo": "Entre 29 y 15 días antes", "penalidad": "25% del total"},
    ]
...
def test_validate_cancellation_policies_invalid(validator):
    ...
    with pytest.raises(
        ValidationError, match="Política debe tener periodo y penalidad"
    ):
...
def test_validate_search_criteria_success(validator):
    ...
    criteria = {"destino": "Costa Mujeres", "fechas": ["2025-03-15", "2025-03-22"]}
