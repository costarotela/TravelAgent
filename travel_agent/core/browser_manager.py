...
class BrowserManager:
    ...
    def __init__(
        ...
        # Configurar LLM
        self.llm = ChatOpenAI(model=llm_model, api_key=config.OPENAI_API_KEY)
    ...
    async def extract_data(
        self, url: str, extraction_template: Dict[str, Any], use_cache: bool = True
    ) -> ExtractionResult:
        ...
        try:
            ...
            if use_cache:
                ...
                if cached_data:
                    return ExtractionResult(success=True, data=cached_data, errors=[])
            ...
            async with self.get_browser() as browser:
                ...
                result = await self.agent.run(page=page, task=extraction_template)
                ...
                if result.success:
                    ...
                    return ExtractionResult(success=True, data=result.data, errors=[])
                else:
                    return ExtractionResult(
                        success=False, data={}, errors=[str(result.error)]
                    )

        except Exception as e:
            self.logger.error(f"Error extrayendo datos: {str(e)}")
            return ExtractionResult(success=False, data={}, errors=[str(e)])
