...
class OpportunityTracker:
    ...
    async def track_opportunities(
        self, packages: List[TravelPackage], criteria: Optional[SearchCriteria] = None
    ) -> List[Opportunity]:
        """
        ...
        try:
            ...
            for package in packages:
                ...
                analysis = await self.analyzer.analyze_package(
                    package=package, criteria=criteria.dict() if criteria else {}
                )
                ...
                if self._is_opportunity(analysis, price_alerts):
                    opportunity = await self._create_opportunity(
                        package=package, analysis=analysis, alerts=price_alerts
                    )
    ...
    async def start_tracking(
        self, interval: int = 3600, criteria: Optional[SearchCriteria] = None  # 1 hora
    ):
        """
    ...
    def _is_opportunity(
        self, analysis: Dict[str, Any], alerts: List[Dict[str, Any]]
    ) -> bool:
        ...
        price_drops = [alert for alert in alerts if alert["type"] == "price_drop"]
        if not price_drops:
            return False
    ...
    def _prioritize_opportunities(
        ...
        return sorted(opportunities, key=lambda x: (x.score, x.savings), reverse=True)

    async def _store_opportunities(self, opportunities: List[Opportunity]):
        """Almacenar oportunidades en base de conocimiento."""
        try:
            for opportunity in opportunities:
                await self.memory.store_opportunity(opportunity.dict())
        except Exception as e:
    ...
    def _generate_description(self, package: TravelPackage, savings: float) -> str:
    ...
    def _generate_conditions(
        self, package: TravelPackage, analysis: Dict[str, Any]
    ) -> List[str]:
